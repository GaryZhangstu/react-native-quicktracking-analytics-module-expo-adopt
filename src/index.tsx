 ...
 */
export function init(appKey: string, channel: string) {
 ...
 *  2. 请在主线程中完成预初始化方法的调用
 */
export function preInit(appKey: string, channel: string) {
 ...
 */
export function setTrackDomain(
 ...
 */
export function uploadPageProperties(pageName: string, params: EventParams) {
 ...
 */
export function sendEventForH5(data: string) {
  try {
    ...
    if (Platform.OS === 'ios') {
      ...
      if (json.sdkArgs.id === '$$_page_start') {
        QuicktrackingAnalyticsModule.onEventForH5(
          JSON.stringify({
            params: json,
            methodName: 'sendPV',
            sid: `${Date.now()}`,
 ...
 */
export function registerGlobalProperty(gp: GlobalProperty) {
  if (Platform.OS === 'ios') {
 ...
 */
export function unregisterGlobalProperty(propertyName: string) {
 ...
 */
export const getGlobalProperty = async (propertyName: string) => {
  try {
    const value =
      await QuicktrackingAnalyticsModule.getGlobalProperty(propertyName);
    return value;
  } catch (e) {
    console.log(e);
  }
 ...
 * @param provider
 */
export const profileSignIn = (ID: string, Provider?: string) => {
  switch (Platform.OS) {
    case 'android': {
      if (Provider) {
 ...
 * 设置应用版本
 */
export const setAppVersion = (version: string, versionCode: number) => {
  if (Platform.OS === 'android') {
 ...
 * 自定义设备ID
 */
export const setCustomDeviceId = (deviceId: string) => {
